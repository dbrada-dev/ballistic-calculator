...
/**
 * Handles ballistic curve points
 * @param curve holds points
 */
public record BallisticCurve(Node[] curve) {

    /**
     * Constructs the curve
     * @param curve calculator output
     */
    public BallisticCurve(List<double[]> curve) {
        this(new Node[curve.size()]);

        for (int i = 0; i < this.curve.length; i++) {
            this.curve[i] = new Node(curve.get(i));
        }
    }
